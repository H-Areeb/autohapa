...
//----------- on count function ---------------------------------------------------
    ...
    if(isset($_GET['count_user_id']))
    {
        $count_user_id = $_GET['count_user_id'];
        $count_ = array();
        $count_approved = mysqli_query($conni,'SELECT count(id) AS active FROM `car_ad` WHERE `isadminapproved_id` = 1 AND  `isdeleteyn_id` = 2 AND `type_id` = 1 AND `iscompleted` = 1  AND  customer_id = "'. $count_user_id.'" ');
             
            $count_ []= mysqli_fetch_assoc($count_approved);

        $count_rejected = mysqli_query($conni,'SELECT count(id) AS rejected FROM `car_ad` WHERE `isadminapproved_id` = 2 AND  `isdeleteyn_id` = 1 AND `type_id` = 1 AND `iscompleted` = 1  AND  customer_id = "'. $count_user_id.'"');
                
            $count_[] = mysqli_fetch_assoc($count_rejected);

        $count_waiting = mysqli_query($conni,'SELECT count(id) AS waiting FROM `car_ad` WHERE `isadminapproved_id` = 2 AND  `isdeleteyn_id` = 2 AND `type_id` = 1 AND `iscompleted` = 1  AND  customer_id = "'. $count_user_id.'"');
            
            $count_[] = mysqli_fetch_assoc($count_waiting);

            

            echo(json_encode(array("data"=>$count_)));
    }
...
if (isset($_POST['approved_id']) && isset($_POST['rejected_id'])) {
    ...
    $CarSearchQuery = mysqli_query(
          ...
          'SELECT car_ad.`id` AS id , car_ad.`adtitle` AS title , car_ad.`adverttext` AS Detail , car_lkptColour.`name` AS color , car_variant.`name`  AS variant ,
              ...
              INNER JOIN car_lkpttransmission ON car_ad.`car_transmissionid` = car_lkpttransmission.`id`
              INNER JOIN car_images ON car_images.`carad_id` = car_ad.`id` AND car_images.`ordinal` = 0
               WHERE  car_ad.`customer_id` = ' . $customer_id . ' AND car_ad.`isadminapproved_id` = ' . $approved_id . '
                AND car_ad.`isdeleteyn_id` = ' . $rejected_id . '
                ORDER BY car_ad.`id` DESC '
    );
